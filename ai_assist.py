            ...
            "items": {
                ...
                "properties": {
                    ...
                    "breakdown": {"type": "array",
                                  "description": "カテゴリ内の具体的な内訳と件数。保有資格の不足→不足している資格・許可の正式名称ごと（例: 電気工事施工管理技士 4件／建設業許可（建築工事業） 2件）、等級・ランク不足→必要だった発注機関・等級ごと（例: 国交省B等級 2件）、地域要件→要求された地域名ごと（例: 北海道 3件）。読み取れる場合は必ず出す",
                                  "items": {"type": "object", "properties": {
                                      "item": {"type": "string", "description": "内訳名（資格名・地域名など）"},
...
_NG_REASONS_SYSTEM = (
    ...
    "count の合計が案件数と一致するようにすること。"
    "さらに各カテゴリの breakdown に具体的な内訳を出すこと: "
    "『保有資格の不足』は不足している資格・許可の正式名称ごとの件数"
    "（例: 電気工事施工管理技士 4件 / 管工事施工管理技士 2件 / 建設業許可（建築工事業） 1件）。"
    "メモの砕けた表現（『建築の許可がない』『土木の実績が要る』等）は"
    "正式名称（建設業許可（建築工事業）・建設業許可（土木工事業） 等）に正規化すること。"
    "『等級・ランク不足』は必要だった発注機関・等級ごとの件数"
    "（例: 国交省B等級 2件 / 大阪府A等級 1件）。機関名か等級の片方しか"
    "読み取れない場合は読み取れた方だけで内訳を出す。"
    "『地域要件』は要求された地域名（都道府県・市区町村・地方名）ごとの件数"
    "（例: 北海道 3件 / 京都市 2件）。"
    "内訳はメモ・案件名・発注機関名から特定できる範囲で出し、根拠のない推測はしない。"
    "どうしても読み取れない場合のみ breakdown を空にする。日本語で出力。"
)
