            ...
            "items": {
                ...
                "properties": {
                    ...
                    "breakdown": {"type": "array",
                                  "description": "カテゴリ内の具体的な内訳と件数。保有資格の不足→不足している資格名ごと（例: 電気工事施工管理技士 4件）、地域要件→要求された地域名ごと（例: 北海道 3件）。読み取れる場合は必ず出す",
...
_NG_REASONS_SYSTEM = (
    ...
    "『保有資格の不足』は不足している資格の名称ごとの件数"
    "（例: 電気工事施工管理技士 4件 / 管工事施工管理技士 2件 / 電気主任技術者 1件）、"
    "『地域要件』は要求された地域名（都道府県・市区町村・地方名）ごとの件数"
    "（例: 北海道 3件 / 京都市 2件）。メモから読み取れる限り具体的に。"
    "資格名や地域名がメモに無い場合は breakdown を空にする（推測しない）。日本語で出力。"
)
