...
# 「変圧器」「火災報知」「火災受信機」は表に無かったため、案件名が明らかに電気なのに
# その他へ落ちていた（実データ: 京都市「久世地域体育館における変圧器等整備業務委託」、
# 陸自「１１１号建物火災受信機更新工事」）。2026-08-28 追加。
_CATEGORY_RULES: list[tuple[str, tuple[str, ...]]] = [
    ...
    ("電気工事-受変電", ("受変電", "変電", "変圧器", "キュービクル", "高圧受電",
                         "特高", "電力量計")),
    ...
    ("電気工事-電気設備", ("電気設備", "電気工事", "受変電設備", "動力設備",
                           "配電", "幹線", "分電", "受電設備", "電気工作物",
                           "自家用電気", "火災報知", "火災受信機")),
    # --- 建築（電気の後ろに置く：分離発注の電気ぶんを奪わないため） ---
    # 「◯◯小学校トイレ改修工事」のような建築の案件は、案件名に業種語が1つも無いため
    # 説明文の電気語（内訳の電灯設備工 等）を拾って電気に化けていた。
    # **必ず電気系ルールより後ろに置くこと**。前に置くと
    # 「望洋台中学校校舎等トイレ改修電気設備工事」「トイレ電気設備」のような
    # 電気の分離発注ぶん（本番に319件ある）まで建築に取られる。2026-08-29 追加。
    ("建築・改修", ("トイレ", "便所", "内装", "外壁", "屋根", "模様替", "浴場",
                    "シャワー", "洗面", "建具", "サッシ", "昇降機", "エレベーター",
                    "増築", "耐震補強", "建築一式")),
]
# タイトルがこの業種に当たったときだけは、公告の業種欄があればそちらを優先する
# （同じ建物工事でも電気の分離発注ぶんが別にあるため）。
_BUILDING_CATEGORY = "建築・改修"
# 案件名に「設備」とあるものは分離発注の設備ぶん（機械設備/電気設備）なので、
# 建物の語が入っていても建築とは決めつけず、従来どおり業種欄・説明文で判断する。
# 実例:「交通センター受付棟増築（設備）工事」「トイレリニューアル工事(機械設備工事)」。
# ※「バリアフリー」は建築ではないものが多いため語から外した
...
# ------------------------------------------------------------
# 例（京都市 電子入札）:「… 開札日 2026.08.25 種目 清掃 内容 その他（清掃） 要求課 …」
# 発注者自身の申告なので、本文のキーワード拾いより桁違いに信頼できる。
# 欄の値が _CATEGORY_RULES のどれにも当たらないとき（「建物管理」「電気機械・器具」等）は
# 採用せず従来どおり本文フォールバックへ落とす＝分からないものを勝手に決めない。
_DECLARED_LABELS = ("工事種別", "工事種類", "業務種別", "営業種目",
                    "種目", "工種", "業種", "種別")
_DECLARED_RE = re.compile(
    r"(?:" + "|".join(_DECLARED_LABELS) + r")\s*[:：]?\s*([^\s　:：]{1,24})")
# 値の頭に付く飾り。「種目 （清掃）」のような書き方を吸収する。
_DECLARED_TRIM = "（()【】「」［］[]・.,、:："
# 欄を探す範囲。公告のヘッダ（案件名・入札方式・種目…）は説明文の先頭に固まっている。
# 全文を見ると添付仕様書の中の「業種」等まで拾ってしまうので、頭だけに限る。
_DECLARED_SCAN_CHARS = 2500

# ------------------------------------------------------------
# 本文フォールバックの誤爆ガード
# ------------------------------------------------------------
# 官公需APIの説明文には添付の仕様書・様式・点検表まで丸ごと入る（1万字級）。
# そのため清掃委託の点検表にある「照明センサー点検」「照明の電球が切れていないか」を
# 拾って電気案件に化ける。実例（2026-08-28 川野さん指摘）: 京都市
# 「公衆トイレ応急対応業務委託」＝種目 清掃 が「電気工事-照明」になっていた。
# 文書全体が明らかに別業種の話をしているとき（非電気の語が _BODY_MIN_HITS 回以上、
# かつ電気の語の _BODY_RATIO 倍以上）だけ、その業種に譲る。
# 単発の付随記述では発火しないので、本文判定を非電気へ広げたときの誤爆
# （改修/増築工事が清掃/造園/管に化ける）は起きない。
_BODY_MIN_HITS = 3
_BODY_RATIO = 2
# 多数決ガードを効かせない合図＝案件名に電気の匂いがある場合。
# 案件名は最も信頼できる情報なので、キーワード表に無い言い回し（「電気中央監視装置」
# 「灯台施設」等）でも電気の語が入っていれば、本文の多数決で非電気へ落とさない。
# 実データで確認: 「和歌山県動物愛護センター電気中央監視装置保守点検業務委託」が
# 施設の建物管理仕様（清掃の記述だらけ）に引きずられて清掃・廃棄物へ落ちていた。
_ELEC_TITLE_HINTS = (
    "電気", "電灯", "電線", "電源", "電力", "電設", "照明", "ＬＥＤ", "LED",
    "受変電", "変電", "変圧器", "キュービクル", "発電", "太陽光", "蓄電",
    "配電", "分電", "幹線", "動力", "計装", "制御", "監視", "通信", "放送", "灯",
    # ケーブル布設・架空線・光配線など、表に無いが明らかに電気の役務を守る
    "ケーブル", "架線", "架空線", "電柱", "光ファイバ", "火災報知", "受信機",
)
# 重なる語（"給排水" と "排水" 等）を二重に数えないよう、長い語優先の非重複マッチで数える。
_RULE_RE = {
    name: re.compile("|".join(re.escape(k) for k in sorted(kws, key=len, reverse=True)))
    for name, kws in _CATEGORY_RULES
}


def _declared_category(text: str) -> str:
    """公告が明記する「種目/工種/業種」欄から業種を読む。読めなければ ""。

    値が業種名で【始まっている】ときだけ採用する。「業種」の2文字は
    「本案件と同一の業種について、建設業許可を有すること」のような地の文にも出るため、
    値に業種名が含まれるだけで採ると参加資格の文章を欄と誤読する（実データで確認）。
    「種目 清掃」「工事種別 土木一式工事」「工種 電気工事(2)営業所の…」は頭で当たる。
    """
    for m in _DECLARED_RE.finditer(text[:_DECLARED_SCAN_CHARS]):
        value = m.group(1).lstrip(_DECLARED_TRIM)
        for name, keywords in _CATEGORY_RULES:
            if any(value.startswith(k) for k in keywords):
                return name
    return ""


def _dominant_non_electrical(text: str) -> str:
    """本文が電気より明確に別業種を語っているならその業種名、でなければ ""。"""
    elec_hits = sum(len(_RULE_RE[n].findall(text))
                    for n, _ in _CATEGORY_RULES if n.startswith("電気工事"))
    best, best_hits = "", 0
    for name, _ in _CATEGORY_RULES:
        # 建築の語（外壁・屋根・内装 等）は本文では「場所」を指すただの言葉として
        # どの仕様書にも出るので、多数決の材料にしない。案件名と業種欄でだけ使う。
        # 実例:「光ケーブル新設役務」の布設ルート表に「外壁沿い」「外壁～電柱」が
        # 6回出て、電気の役務が建築に化けた。
        if name.startswith("電気工事") or name == _BUILDING_CATEGORY:
            continue
        hits = len(_RULE_RE[name].findall(text))
        if hits > best_hits:
            best, best_hits = name, hits
    if best_hits >= _BODY_MIN_HITS and best_hits >= _BODY_RATIO * elec_hits:
        return best
    return ""
...
def classify_category(text: str, title: str = "") -> str:
    ...
    判定は信頼できる順に4段:
      1. 案件名のキーワード（最も信頼できる）
      2. 公告が明記する「種目/工種/業種」欄（発注者自身の申告）
      3. 説明文の電気系キーワード（本業だけは本文でも信頼できる）
         ただし文書全体が明らかに別業種なら、その業種に譲る
      4. その他
    """
    ...
    if declared:
        return declared
    # 3) それも無ければ説明文を見る。ただし【電気系(本業)に限定】する。
    ...
    for name, keywords in _CATEGORY_RULES:
        if name.startswith("電気工事") and any(k in text for k in keywords):
            if any(h in t for h in _ELEC_TITLE_HINTS):
                # 案件名が電気を示している以上、本文の多数決には譲らない
                return name
            # 付属仕様書に紛れた一言で電気に化けるのを防ぐ（_dominant_non_electrical 参照）
            return _dominant_non_electrical(text) or name
    return "その他"
